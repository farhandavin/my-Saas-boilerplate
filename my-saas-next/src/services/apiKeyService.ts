...
// Type for API key insert values
...
export const ApiKeyService = {
  ...
  async createKey(teamId: string, name: string) {
    ...
    // 4. Simpan ke DB with properly typed values
    const insertValues: ApiKeyInsertValues = {
      teamId,
      name,
      prefix,
      keyHash
    ...
    const [apiKeyRecord] = await db.insert(apiKeys).values(insertValues).returning();
